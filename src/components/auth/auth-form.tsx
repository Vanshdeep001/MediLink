...
export function AuthForm() {
  ...
  const onRegisterSubmit = (values: z.infer<typeof registrationSchema>) => {
    try {
      ...
      const user = { ...values, age, role: 'patient' };
    ...
    <div className="w-full max-w-lg mx-auto">
      <div className="text-center mb-8">
        <h1 className="text-4xl md:text-5xl font-bold tracking-tight whitespace-nowrap">
          <TextFlipper>{translations.authPortal.patient.titleMain}</TextFlipper>{' '}
          <TextFlipper delay={0.2} className="text-primary font-cursive">{translations.authPortal.patient.titleCursive}</TextFlipper>
        </h1>
      </div>
      <Card>
          ...
          <TabsContent value="register" className="mt-6">
              <Form {...registrationForm}>
                  ...
                  <FadeIn delay={400} direction="right">
                      ...
                      render={({ field }) => (
                          <FormItem>
                          <FormLabel>{translations.authForm.passwordLabel}</FormLabel>
                              <div className="relative">
                      ...
                      render={({ field }) => (
                          <FormItem>
                          <FormLabel>{translations.authForm.confirmPasswordLabel}</FormLabel>
                              <div className="relative">
                              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                              <FormControl><Input type="password" className="pl-10 h-12" {...field} /></FormControl>
                          </div>
